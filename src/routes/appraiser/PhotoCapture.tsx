...
import { InspectionState, InspectionType, CapturedPhoto } from '../../types/photoCapture';
import { getActiveSlots, getCurrentSlot, canCompleteInspection, getProgressStats } from '../../utils/photoCapture';
  ...
  const completeInspection = async () => {
    ...
    if (!uploadStatus.allComplete) {
      const confirm = window.confirm(
        `${uploadStatus.uploading + uploadStatus.pending} photos are still uploading.\n\n` +
    ...
    return (
      <div style={{
        ...
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <h2 style={{ color: '#e2e8f0', marginBottom: 20 }}>Select Inspection Type</h2>
  ...
  if (showLabelPrompt) {
    return (
      <div style={{
        ...
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <h2 style={{ color: '#e2e8f0', marginBottom: 20 }}>Manufacturer/Specialty Label</h2>
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ margin: '5px 0 0 0', color: '#cbd5e0', fontSize: 14 }}>
                {state.inspection_type === 'regular' ? '🚗 Regular Vehicle' : '🚛 Heavy Duty'}
              </p>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ color: '#e2e8f0', fontSize: 20, fontWeight: 'bold' }}>
                {progress.completed} / {progress.total}
              </div>
              <div style={{ color: '#cbd5e0', fontSize: 12 }}>photos complete</div>
            </div>
          </div>
          ...
          {!uploadStatus.allComplete && (
            <div style={{
              ...
              ↑ Uploading {uploadStatus.uploading + uploadStatus.pending} photo(s) safely in background...
            </div>
          )}
          ...
          {uploadStatus.failed > 0 && (
            <div style={{
              ...
              ⚠️ {uploadStatus.failed} photo(s) failed to upload. Check connection.
            </div>
          )}
        </div>
        ...
        <div style={{
          ...
          <h3 style={{ color: '#e2e8f0', fontSize: 16, marginTop: 0 }}>Additional Sections (If Applicable)</h3>

          {['structural', 'airbags', 'tow_bill'].map(group => (
            <label key={group} style={{ display: 'flex', alignItems: 'center', marginBottom: 10, cursor: 'pointer' }}>
              <input
                type="checkbox"
        ...
        {currentSlot && (
          <div style={{
            ...
            <h3 style={{ color: '#e2e8f0', marginTop: 0 }}>
              {currentSlot.label}
              {currentSlot.required && <span style={{ color: '#ef4444' }}> *</span>}
            </h3>
            <p style={{ color: '#cbd5e0', marginBottom: 20 }}>{currentSlot.instruction}</p>

            {/* Captured photos for this slot */}
            {state.captured_photos.get(currentSlot.id)?.map(photo => (
              <div key={photo.id} style={{ marginBottom: 15, position: 'relative' }}>
                <img src={photo.url} alt="Captured" style={{ width: '100%', borderRadius: 8 }} />
                {!state.completed && (
                  <button
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {!cameraActive ? (
                <>
                  ...
                  {currentSlot.max_photos === -1 && state.captured_photos.get(currentSlot.id)?.length > 0 && (
                    <button
                      onClick={() => {
                        const nextIndex = currentSlotIndex + 1;
                        if (nextIndex < activeSlots.length) {
            ...
            {activeSlots.map((slot, idx) => {
              ...
              return (
                <button
                  ...
                  style={{
                    padding: 10,
                    background: isCurrent ? '#667eea' : (isComplete ? '#10b981' : '#4a5568'),
                    color: 'white',
                    border: 'none',
                    borderRadius: 6,
                    fontSize: 12,
